...
export default function NotaEDM({ article }: Props) {
  ...
  return (
    <article className="flex h-full w-full items-stretch gap-2 p-1.5">
      {snapshot.imageUrl && (
        <div className="aspect-video max-w-[40%] h-full shrink-0 overflow-hidden bg-[var(--surface-secondary)]">
          <img
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between gap-1">
        <h2 className="m-0 line-clamp-3 text-[16px] leading-tight text-white">
          {snapshot.title}
        </h2>
        {snapshot.volanta && (
          <span className="text-[11px] font-semibold uppercase tracking-tight text-red-600">
            {snapshot.volanta}
          </span>
        )}
      </div>
